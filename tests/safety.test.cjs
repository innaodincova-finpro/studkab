...
async function cloud(){
 ...
 c.supabase={createClient:()=>({auth:{onAuthStateChange(fn){c.authEvent=fn},getSession:async()=>({data:{session:{user:c.user,access_token:'public-user-token'}}}),signOut:async()=>c.logout||{},signInWithPassword:async args=>{c.passwordArgs=args;return c.passwordResult||{data:{user:c.user}}},updateUser:async args=>{c.passwordUpdate=args;if(c.passwordThrow)throw c.passwordThrow;return c.passwordResponse||{data:{user:c.user}}},signInWithOAuth:async args=>{c.oauth=args;return c.oauthResult||{}},verifyOtp:async()=>({data:{user:c.user}})},from:()=>q,rpc:(name,args)=>{c.calls.push({name,args});return Promise.resolve(c.nextWrite)},storage:{from:bucket=>({upload:async(path,file,options)=>{c.storageCall={bucket,path,file,options};return c.storageResult||{data:{path}};},download:async path=>{c.downloadCall={bucket,path};return c.downloadResult||{data:new Blob(['docx'])};}})}})};
...
test('workflow client uses the signed-in token and private non-overwriting upload',async()=>{
 ...
 const blob=await c.Oblako.workflowDownload('request/result/1');assert.equal(await blob.text(),'docx');assert.equal(c.downloadCall.bucket,'studkab-private');
});
...
test('quality workflow UI is present in both roles and parses independently',()=>{
 ...
 assert.match(read('sw.js'),/workflow-ui\.js\?v=4/);
});

...
test('executor reviews Word, retries safely and receives an honest delivery confirmation',async({page})=>{
 ...
 await page.evaluate(()=>{
  ...
  const candidate={id:'11111111-1111-4111-8111-111111111111',requestNumber:1,topic:'Проверка документа',student:'Тестовый студент',format:{},doc:{inputs:{requirements:'Методические требования кафедры менеджмента'},order:[{id:'intro',name:'Введение'}],structure:{intro:{text:'Проверенный черновик'}}}};candidate.doc.review=DraftQuality.stamp(candidate);StudResults.deliver(candidate);
 });
 await expect(page.getByText('Требования методички и правила оформления выполнены',{exact:true})).toHaveCount(1);
 await expect(page.getByText('Прибыль и денежные потоки',{exact:true})).toHaveCount(0);
 await expect(page.getByText('Методические требования кафедры менеджмента',{exact:true})).toHaveCount(1);
 await page.getByRole('button',{name:'Завершить проверку и передать',exact:true}).click();await expect(page.locator('[data-result-status]')).toContainText('Проверьте документ');expect(await page.evaluate(()=>deliveries.length)).toBe(0);
...
test('editing a recipient or document after opening delivery prevents sending',async({page})=>{
 ...
 await page.evaluate(()=>{
  ...
  window.candidate={id:'11111111-1111-4111-8111-111111111111',requestNumber:1,topic:'Проверка',student:'Получатель',format:{},doc:{inputs:{requirements:'Требования преподавателя'},order:[{id:'intro',name:'Введение'}],structure:{intro:{text:'Проверенный текст'}}}};
 ...
 await page.getByRole('button',{name:'Завершить проверку и передать',exact:true}).click();

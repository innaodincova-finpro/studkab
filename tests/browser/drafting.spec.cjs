...
async function setup(page){await page.goto('http://127.0.0.1:4173/reestr.html');await page.evaluate(()=>QA.switchUser('draft-editor'));await expect.poll(()=>page.evaluate(()=>Oblako.canSync()&&!Oblako.busy)).toBe(true);await page.evaluate(()=>{const x=fromPayload({id:'33333333-3333-4333-8333-333333333333',t:'Анализ финансового состояния предприятия',k:'Курсовая работа',n:'Тест'});x.topic='Анализ финансового состояния предприятия';x.group='Э-1';x.attachments=[{id:'a1',file_name:'задание.pdf',category:'requirements',size_bytes:100}];x.passports=[{id:'11111111-1111-4111-8111-111111111111',revision:1,status:'approved',title:'Требования',summary:'',items:[{id:'STRUCTURE',verified:true,source:'Задание',text:'Структура: Введение\n1. Анализ исходных данных\nЗаключение'}]}];D.items.push(x);window.draftItem=x;docOf(x);aiReady=()=>true;window.calls=[];window.actualAskAI=askAI;askAI=async(p,s,u)=>{calls.push({s,u});return{text:'Содержательный текст раздела на основе предоставленных материалов.',tokens:1};};Oblako.requestApi=async body=>{if(body.action==='attachment-context')return {attachments:[]};if(body.action!=='passport-ensure')throw Error('Unexpected request action');return {passports:x.passports};};openDocBuilder(x.id);});}

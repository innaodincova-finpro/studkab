...
test('C077 new revision edits filled requirements and keeps a separate draft',async({page})=>{
 ...
 const dialog=page.getByRole('dialog',{name:'Новая версия требований'});
 ...
 await dialog.getByLabel('Основание изменения').fill('Исходное задание требует минимум 10 источников.');
 await dialog.getByRole('button',{name:'Сохранить уточнения'}).click();
 ...
 expect(saved.body.passport.items[1]).toEqual({id:'P2',category:'measurable',required:true,source:'Методичка',text:'Объём 25–30 страниц'});

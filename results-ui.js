(function(global){
 ...
 function deliver(x){
  ...
  wrap.querySelector('[data-deliver]').onclick=async function(){
   ...
   var criteria={};for(var i=0;i<codes.length;i++){var status=wrap.querySelector('[data-criterion-status="'+codes[i]+'"]').value,evidence=wrap.querySelector('[data-criterion="'+codes[i]+'"]').value.trim();if(!status){msg.textContent='Выберите результат: '+labels[i];wrap.querySelector('details').open=true;return;}if(evidence.length<10){msg.textContent='Добавьте доказательство: '+labels[i];wrap.querySelector('details').open=true;return;}if(status==='fail'){msg.textContent='Передача заблокирована: не пройден пункт «'+labels[i]+'».';return;}if(status==='manual'){msg.textContent='Передача заблокирована: завершите ручную проверку «'+labels[i]+'».';return;}criteria[codes[i]]={status:status,evidence:evidence};}

...
(function(global){
 ...
 function passwordField(id,label,autocomplete){
  return '<div class="fld"><label style="font-size:15px" for="'+id+'">'+label+'</label><div style="display:flex;gap:8px;align-items:center"><input style="min-width:0;flex:1;width:100%;font-size:16px;min-height:44px" id="'+id+'" type="password" autocomplete="'+autocomplete+'" '+(autocomplete==='new-password'?'minlength="8" ':'')+'required><button type="button" class="chip" style="width:auto;margin:0;min-height:44px;flex-shrink:0" data-password-toggle="'+id+'" aria-controls="'+id+'" aria-label="Показать пароль: '+label+'" aria-pressed="false">Показать</button></div></div>';
 }
 ...
 function invitationMessage(email,url,kind){
  ...
  var intro=kind==='recovery'?'Восстановление доступа к «Кабинету студента».':'Приглашение в «Кабинет студента». Здесь можно вести учебные работы и отправлять исполнителю заявки на подготовку черновика.';
  var start=kind==='existing'?
   '1. Войдите в кабинет\n'+home+'\nОткройте «Профиль» → «Хранение записей». Войдите через Google с этой почтой или по почте и паролю приложения. Если вы пользуетесь «Точкой дня» с этой почтой, пароль тот же. Новый аккаунт создавать не нужно. Если не получается войти, обратитесь к исполнителю в переписке, в которой получено приглашение.':
   '1. '+(kind==='recovery'?'Восстановите доступ':'Откройте личную ссылку')+'\n'+url+'\n'+(kind==='recovery'?'Задайте новый':'Придумайте')+' пароль приложения — не менее 8 символов. Кнопка «Показать» позволяет проверить ввод. Повторите пароль и нажмите «Сохранить пароль и открыть приложение». Пароль от почтового ящика вводить не нужно; никому не сообщайте пароль приложения.\n'+(kind==='recovery'?'Если с этой почтой вы пользуетесь «Точкой дня», её пароль также изменится.\n':'')+'Если ссылка истекла или не открывается, запросите новую у исполнителя.';
  return intro+'\n\nВаша почта для входа: '+email+'\n\n'+start+
   ...
   '\n\nСохраните это сообщение. '+(kind==='existing'?'':'Личная ссылка предназначена только вам — не пересылайте её другим. ')+'Если нужна помощь, обратитесь к исполнителю в переписке, в которой получено это сообщение. Укажите, на каком шаге возник вопрос. Не отправляйте пароль или снимок личной ссылки.';
 }

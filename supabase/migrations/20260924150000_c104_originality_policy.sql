-- C104: new approvals require an explicit rule. Existing approved passports remain immutable;
-- to use the new path the executor creates and approves a new version.
create function public.studkab_originality_rule(item jsonb) returns jsonb language plpgsql immutable security invoker set search_path='' as $$
...
begin
 ...
 if mode is null or mode not in ('university_threshold','university_no_threshold','service_only') or jsonb_typeof(o) is distinct from 'object'
 then raise exception 'QUALITY_PASSPORT'; end if;
...
revoke all on function public.studkab_originality_rule(jsonb) from public,anon,authenticated;
grant execute on function public.studkab_originality_rule(jsonb) to service_role;
...
begin
 ...
 if new.kind='external_originality' then
  ...
  if c#>>'{thresholdRequirement,mode}'='university_threshold' then
   if x->>'service' is distinct from c#>>'{thresholdRequirement,service}'
...
-- Roles remain service-only; existing output/delivery triggers call the replaced context and guard.

do $guard$ begin if exists(select 1 from public.app_data where app in ('kabinet','reestr')) then raise exception 'New platform data requires a fresh backup before migration'; end if; end $guard$;

-- C-063: qualify the scalar produced by unnest. The prior alias was parsed as
-- a record in production and made dispatch fail before any provider request.
...
begin
 ...
 where exists(select 1 from public.studkab_gen_attempts a where a.job_id=p_job and a.request_id=any(r.request_ids))
   and not exists(select 1 from unnest(r.request_ids) as q(request_id)
     where not exists(select 1 from public.studkab_gen_attempts a where a.job_id=p_job and a.request_id=q.request_id));
 used=used-released;

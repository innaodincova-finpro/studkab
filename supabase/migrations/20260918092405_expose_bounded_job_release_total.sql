-- C-064: the runner must not receive direct access to immutable reconciliation
-- evidence. Expose only the aggregate release for one job through a private,
-- narrowly granted helper.
create schema if not exists studkab_private;
revoke all on schema studkab_private from public,anon,authenticated;
grant usage on schema studkab_private to service_role;

create or replace function studkab_private.gen_job_released(p_job uuid)
returns bigint language sql stable security definer set search_path='' as $$
 select coalesce(sum(r.released_microusd),0)
 from public.studkab_gen_reconciliations r
 where exists(select 1 from public.studkab_gen_attempts a
   where a.job_id=p_job and a.request_id=any(r.request_ids))
 and not exists(select 1 from unnest(r.request_ids) as q(request_id)
   where not exists(select 1 from public.studkab_gen_attempts a
     where a.job_id=p_job and a.request_id=q.request_id))
$$;
revoke all on function studkab_private.gen_job_released(uuid) from public,anon,authenticated;
grant execute on function studkab_private.gen_job_released(uuid) to service_role;

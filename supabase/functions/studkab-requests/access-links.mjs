...
export async function accessLink({base,key,email,recovery=false,request=fetch,isMember}){
 ...
 // C-054: восстановление только для допущенных студентов; для остальных ответ как для отсутствующего.
 if(recovery&&(!found||typeof isMember!=='function'||(await isMember(found.id))!==true))return {missing:true};
 if(!recovery&&found&&(found.email_confirmed_at||found.last_sign_in_at))return {existing:true,userId:found.id};
 ...
 return {url:'https://innaodincova-finpro.github.io/studkab/activate.html#token='+encodeURIComponent(link.hashed_token)+'&email='+encodeURIComponent(email)+(recovery?'&type=recovery':''),userId:found?.id||link.id||link.user?.id};
}

(function(global){
 ...
 var next={submitted:['completeness_review','Начать проверку материалов'],completeness_review:['passport_draft','Сформировать паспорт требований'],passport_approved:['preparing','Начать подготовку'],quality_review:['changes_required','Вернуть на доработку'],changes_required:['preparing','Вернуть в подготовку'],delivered:['closed','Закрыть заявку']};
 var baseRequirements=[
  ...
  ['DOC-01','Титульный лист соответствует паспорту','critical','manual'],['DOC-04','Таблицы и рисунки читаемы и подписаны','critical','manual'],['DOC-06','Файл Word открыт исполнителем: текст, таблицы и оформление проверены','critical','combined'],['DOC-07','Передаваемая версия совпадает с проверенной','critical','automatic'],['DOC-08','Получатель совпадает с владельцем заявки','critical','automatic'],['DOC-09','Ограничения подготовлены для студента','critical','manual']
 ];
 ...
 function requirementRows(requirements,criteria,documentId,locked){
  ...
  return requirements.filter(function(r){return r.applicability==='applicable';}).map(function(r){var manual=by[r.id+':human'],auto=by[r.id+':automatic'];return '<div class="stat" style="display:block"><b>'+esc(r.code)+' · '+esc(r.rule_text)+'</b><div class="hint">'+esc(r.severity==='critical'?'Критический':r.severity==='substantial'?'Существенный':'Рекомендательный')+' · '+esc(r.verification_method==='automatic'&&r.code!=='DOC-06'?'автоматически':r.verification_method==='combined'||r.code==='DOC-06'?'автоматически и вручную':'вручную')+(auto?' · автопроверка: '+esc(auto.status):'')+'</div>'+((r.verification_method!=='automatic'||r.code==='DOC-06')?'<select '+(locked?'disabled ':'')+'data-workflow-check="'+esc(r.id)+'"><option value="">Не проверено</option><option value="pass"'+(manual&&manual.status==='pass'?' selected':'')+'>Соответствует</option><option value="fail"'+(manual&&manual.status==='fail'?' selected':'')+'>Не соответствует</option><option value="unable_to_verify"'+(manual&&manual.status==='unable_to_verify'?' selected':'')+'>Невозможно проверить</option></select><input '+(locked?'disabled ':'')+'data-workflow-comment="'+esc(r.id)+'" maxlength="1000" placeholder="Комментарий или доказательство" value="'+esc(manual&&manual.comment||'')+'">':'')+'</div>';}).join('');
 }
 function executor(data,id){
  ...
  var document=(data.documents||[]).find(function(d){return d.id===data.process.active_document_id;}); if(document)requirements=(data.requirements||[]).filter(function(r){return r.passport_id===document.passport_id;});
  ...
  var doc='<div class="card"><div class="label">Контрольный лист документа</div>'+(document?'<div class="stat"><span>Версия Word</span><b>'+esc(document.version)+' · '+esc(document.state)+'</b></div><details'+(data.process.status==='quality_review'?' open':'')+'><summary>Критерии · '+requirements.length+'</summary>'+requirementRows(requirements,criteria,document.id,data.process.status!=='quality_review')+'</details><div class="chips"><button type="button" class="chip"'+(data.process.status==='quality_review'?'':' hidden')+' data-workflow-action="automatic-checks" data-request-id="'+id+'" data-document-id="'+document.id+'">Запустить автопроверку</button><button type="button" class="chip"'+(data.process.status==='quality_review'?'':' hidden')+' data-workflow-action="checks-save" data-request-id="'+id+'" data-document-id="'+document.id+'">Сохранить ручную проверку</button>'+(data.process.status==='quality_review'?'<button type="button" class="btn" data-workflow-action="document-approve" data-request-id="'+id+'" data-document-id="'+document.id+'" data-revision="'+data.process.revision+'">Подтвердить готовность</button>':'')+(data.process.status==='ready_to_deliver'?'<button type="button" class="btn" data-workflow-action="document-deliver" data-request-id="'+id+'" data-document-id="'+document.id+'" data-revision="'+data.process.revision+'">Передать студенту</button>':'')+'</div>':'<p class="hint">Сначала утвердите паспорт и подготовьте документ.</p>'+(data.process.status==='preparing'?'<button type="button" class="btn" data-workflow-action="document-register" data-request-id="'+id+'" data-passport-id="'+esc(data.process.active_passport_id)+'" data-revision="'+data.process.revision+'">Зарегистрировать текущий Word для проверки</button>':''))+'</div>';
  if(document)doc+='<div class="card"><button type="button" class="chip" data-workflow-action="review-download" data-request-id="'+id+'" data-document-id="'+document.id+'">Скачать Word для проверки</button><p class="hint">Скачивается только файл, совпадающий с зарегистрированной версией. Откройте его в Word; результат укажите в DOC-06 с комментарием.</p></div>'; if(document&&data.process.status==='preparing')doc+='<div class="card"><button type="button" class="btn" data-workflow-action="document-register" data-request-id="'+id+'" data-passport-id="'+esc(data.process.active_passport_id)+'" data-revision="'+data.process.revision+'">Зарегистрировать исправленный Word для проверки</button></div>'; return head+pass+ask+doc;
 }
 async function paint(){
  var node=document.getElementById('workflowPanel');if(!node)return;
  var token=++seq,id=node.getAttribute('data-request-id'),role=node.getAttribute('data-role');node.hidden=false;viewState=null;
  ...
  try{
   ...
   var legacy=document.getElementById('legacyDeliveryPanel');if(legacy)legacy.hidden=true; if(role==='student'){var fab=document.getElementById('fab');if(fab)fab.hidden=true;}
  }catch(e){
 ...
 async function act(button){
  ...
  try{
   if(action==='upload'){
    ...
    var reviewRecord=global.item&&global.item(id),reviewDocument=(viewState.documents||[]).find(function(d){return d.id===button.getAttribute('data-document-id');});
    if(!reviewRecord||!reviewDocument)throw Error('Версия документа не найдена');
    var reviewIdentity=global.Oblako.identity(),reviewContent=global.StudResults.snapshot(reviewRecord),reviewBlob=global.ResultDocx(reviewContent,reviewContent.chapters);
    if(await hash(reviewBlob)!==reviewDocument.docx_sha256)throw Error('Текущий Word отличается от зарегистрированной версии. Верните документ на доработку и зарегистрируйте исправленный Word.');
    if(global.Oblako.identity()!==reviewIdentity)throw Error('Аккаунт изменился. Откройте заявку заново.');
    var reviewUrl=URL.createObjectURL(reviewBlob),reviewLink=document.createElement('a');reviewLink.href=reviewUrl;reviewLink.download='Проверка-версии-'+reviewDocument.version+'.docx';document.body.appendChild(reviewLink);reviewLink.click();reviewLink.remove();setTimeout(function(){URL.revokeObjectURL(reviewUrl);},10000);

...
(function(root){
 ...
 function sectionNotes(x,c,text){
  ...
  if(c.id==='ch3'&&financial(x)){
   var metrics=[/ликвидност/i,/автономи/i,/обязательств.{0,25}капитал/i,/оборотн.{0,15}капитал/i,/рентабельност/i].filter(function(re){return re.test(t);}).length;
   if(metrics>=3&&/\d/.test(t))notes.push('Рекомендации повторно перечисляют несколько показателей. Оставьте необходимые ссылки на выводы главы 2 и сосредоточьтесь на действиях.');

...
## C-071 — установка и проверка рабочей среды, 2026-09-19

Пользователь подтвердил установку сообщением «Выполняй следующий этап».
PR #112 объединён: main a88dc434b162c7318552ba524588e8d127e8bdf8;
дерево 158058f7ca68fb3762cc53fa60276a249709e937 совпадает с проверенным.
Safety PR 35444330456 — success: 272 Node, SQL safety, A1, replay, 72 browser.

Установка выполнена в порядке SQL → Edge → frontend. В Supabase применён
точный SQL файла 20260919124300_result_review_context_guard.sql
(SHA-256 7c0279620d2cc348b12561dfe2bce4794d4a046722d3b755a84268eee3acb4fd).
Инструмент установки присвоил фактическую версию 20260919133155
с именем result_review_context_guard. Это соответствие важно при следующей сверке:
не применять локальный файл повторно и не выполнять безусловный db push.
Маркер=1, триггер установлен; anon/authenticated не имеют EXECUTE,
service_role имеет. Накопленные pending-миграции не устанавливались.

Edge установлен перед frontend (v31), затем штатный GitHub deploy подтвердил
v32 ACTIVE: восемь исходных файлов побайтно совпадают с проверенным кодом.
Первый пакет прямой установки не собрался из-за отсутствующего shared-модуля;
после включения существующего secret-equal.mjs установка успешна, код не менялся.
Deploy studkab requests 35446130110 — success.
GitHub Pages 35446129619 — success. reestr.html, index.html, results-ui.js,
request-workflow.js, sw.js получены по HTTPS с проверкой сертификата: HTTP 200,
байты совпадают с проверенной версией. Endpoint без входа возвращает HTTP 401.

В рабочем Postgres выполнена синтетическая транзакция с полным ROLLBACK:
save не создаёт выдачу; stale passport блокирует выдачу; отдельная передача
создаёт одну выдачу; повтор возвращает duplicate=true; версия неизменяема.
Записи реальных студентов не менялись, синтетические строки не сохранялись.
Identity sequences могут иметь пропуски после тестового rollback.
AI, email и Telegram в тесте не вызывались.

Ограничение: это проверка рабочего SQL, опубликованных файлов и отказа без входа,
а не приёмка полного пути в двух авторизованных браузерах. Их сеансов в текущей
среде нет. Проверка UI с mock endpoint и тестовая SQL-транзакция не заменяют эту
приёмку. Следующий шаг — исполнителю войти обычным способом и пройти одну
тестовую заявку вместе с отдельным тестовым студенческим аккаунтом.
Общий TASK.md и приёмки качества/устойчивости остаются открытыми в прежнем объёме.

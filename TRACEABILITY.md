...
R3/R8/R13 → `results-ui.js`, `reestr.html`, `index.html`, `sw.js` и `tests/browser/results.spec.cjs`. Локально изменены надпись кнопки, инструкция, отдельное подтверждение открытия скачанного файла и сброс подтверждения при изменении сохранённого Word. `npm test`: 472/472, `node --check` и `git diff --check` пройдены. Адресный browser-тест подготовлен, но локально Chromium отсутствует; CI и рабочая публикация/приёмка пока не проведены. Заявка №4 и её серверные проверки этой правкой не изменены; утверждать, что замечания сохранены, нельзя.

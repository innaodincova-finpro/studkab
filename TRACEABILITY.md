...
## C106 — подтверждённый порог без названной системы, 24.09.2026

R3/R8/R9/R13 → `requirements.mjs`, `quality-evidence.mjs`, `reestr.html`, `quality-evidence-ui.js`, `20260924150100_originality_threshold_without_service.sql`. Основание №3: `01_assignment.txt` требует 65%, система не указана. Новый режим сохраняет число и точное основание, допускает утверждение паспорта после проверки остальных условий, но запрещает положительное свидетельство и обычную выдачу до установления применимой системы. Старый Word и паспорт №3 не изменены этой программной правкой. Локальные проверки, публикация и фактическое повторное утверждение паспорта фиксируются отдельно.

## C105 — исправление вида работы при сохранённом результате, 24.09.2026

R3/R8/R9/R13. Фактическое основание: в рабочей заявке №3 карточка содержит «Контрольная работа», актуальное задание `01_assignment.txt`, паспорт и Word — «Практическая работа». Обычное редактирование после сохранения результата заблокировано. Локальная ветка `codex/c105-request-kind-correction`: отдельная проверяемая операция `request-kind-correct` в Edge, SQL `20260924043558_c105_request_kind_correction.sql`, форма реестра. Операция сохраняет прежнюю карточку и автора исправления в истории, сверяет действующее задание/редакцию, делает старые паспорта устаревшими, оставляет Word и замечания неизменными. Старый Word не считается прошедшим новую проверку.

Изолированная SQL/API проверка C105: 3/3, в том числе чужой исполнитель, устаревшая редакция, устаревшее задание, повтор операции и история. Основной Node-набор 472/472. Синтаксис обновлённого сценария реестра и `git diff --check` пройдены. Локальный browser-набор не стартовал: исполняемый Chromium отсутствует; обязательная браузерная проверка остаётся в CI. **Рабочая заявка №3 пока не изменена**: SQL, Edge и форма ещё не установлены, повторный паспорт/Word и внешние проверки не выполнены. В этих границах не утверждать C01 как пройденный.

## C104 — порог только по применимому правилу, 24.09.2026

R13/R3/R8/R9. Код локальной ветки: паспорт и редактор `requirements.mjs`,
`reestr.html`; проверка PDF `quality-evidence.mjs`, `quality-evidence-ui.js`;
SQL-защита прямых вызовов и выдачи в pending-миграции
`20260924150000_c104_originality_policy.sql`. При утверждении нового паспорта
требуется один из трёх явных режимов. Установленный вузом порог и система
сверяются с отчётом; при отсутствии числа записывается null, внешний PDF
всё равно обязателен. Попытки снизить процент, сменить систему, подать
чужой/старый отчёт и обойти форму серверным вызовом отклоняются.

Проверки: основной локальный Node/SQL набор 469/469; SQL отдельно 11/11,
в том числе два случая без порога и прямой вызов утверждения. Синтаксис
встроенного JS интерфейса и `git diff --check` пройдены.
После исправления макета окна в тесте GitHub Safety checks №35951798028
завершился success на опубликованном коммите
`6cebe0c2de9fe20f6451d2723dc9f86bb97c9905`: npm test, SQL,
конкурентные сценарии, воспроизведение миграций в изолированном Supabase
и browser-тесты успешны. Черновой PR №149 открыт и совместим с текущей main
по проверке GitHub; production пока не обновлялся. Предварительная сверка
рабочей базы: пять утверждённых паспортов старого формата, две записи
свидетельств качества и семь активных заявок. Прежние паспорта не
переписываются; для нового положительного заключения нужна новая версия.
Миграция, Edge и сайт в production не установлены. Реальный оплаченный
отчёт и независимая предметная проверка
QD01–QD07 также не выполнены. C102/C104 остаются незавершёнными до
публикации и сквозной приёмки. Исторические данные не переписаны.

## C103 — проверка применимости правил к реальной заявке, 24.09.2026

R13/R3/R7/R8/R9. Первичные правила вузов и существующий код сопоставлены;
маршрут и сценарии приёмки записаны в `docs/REAL_REQUEST_QUALITY_PROCESS.md`.
В автоматически созданном паспорте без основания больше не заявляется источник
«Требования вуза»: `supabase/functions/studkab-requests/requirements.mjs` и
`tests/requirement-passports.test.mjs`. Если студент прямо написал, что
порог не задан, история его заявления отдельно остаётся «Заявка студента».
Локальный тест: 12/12, включая это различие и прежнюю блокировку утверждения.

Статус: исследование и правка опубликованы в черновом PR №149 вместе с C104;
технический CI успешен. Установки и полной предметной приёмки нет. Пакет
SQL/Edge/UI для случаев без числового вузовского порога реализован в C104,
но ещё не установлен; конфликт применимых источников остаётся предметом
отдельного вопроса студенту и реальной приёмки. Старые свидетельства не
переписывались.

## C-092 — сверка учёта установок и исторической заявки, 22.09.2026
R3/R9. Read-only запросы в dcpthwmuiodrjepifzsd под postgres/BYPASSRLS:
заявки №8–12; request a47632e9-ff30-426d-9e1a-91979667930f отсутствует;
studkab_request_deletion_audit содержит 0 строк. Журнал не устанавливает время,
способ, причину и автора исчезновения №6. Исторические протоколы ниже сохранены,
но не означают текущей доступности записей №6 или завершённой приёмки.
Все 11 pending-записей manifest найдены в schema_migrations: 10 SQL совпали
по SHA256, диагностика подготовки совпала после исключения комментариев/пробелов.
Дополнительно сверены 5 существующих записей: C054 grants и reserve context
отличаются комментариями/пробелами, три остальные совпадают точно. В manifest
сохранены локальные имена/хеши и добавлены отдельные production_version/sha256,
дата и вид сравнения. Старые неподтверждённые даты не выданы за новую сверку.
Ни миграции, ни данные, ни права, ни серверные функции этой правкой не меняются.
Наличие записи установки не доказывает отсутствие последующих изменений объектов
и не заменяет сквозную приёмку. Повторная установка этих 11 изменений не требуется.
...
C086 установка подтверждена 22.09.2026: PR131 merged 871a726754c57cb5ac33906fd8ad86587b60e844;
Safety35678511058 и Pages35678964178 success. Три публичных файла совпали
побайтно с проверенной веткой; скачанный модуль прошёл сценарии трёх дисциплин.
Платной генерации не было; полная содержательная приёмка не закрыта.

## C-087 — отделение приблизительного объёма, 22.09.2026
R5/R8/R9 → draft-quality.js finAcceptance/riskReport/reviewCriteria, cache v14/sw v75.
Приблизительные диапазоны обычных работ идут в notes и отображаются в отчёте;
errors остаются для действительных блокеров. Пустые разделы и профиль FIN-UAT-01
не ослаблены. C12 уточняет проверку фактических страниц точного Word по заданию.
Локально npm test 328/328 + pretest вложений 13/13; quality 39/39;
git diff --check чисто. Добавлен браузерный сценарий: 300 слов при ориентире
1 страница открывают протокол; C12 manual/fail запрещает сохранение/передачу.
Браузерный сценарий подлежит проверке CI. Main перед изменениями сверена:
871a726754c57cb5ac33906fd8ad86587b60e844, дерево совпадает с локальной базой.
Рабочие заявки, серверные ограничения и AI-расходы не менялись. Публикация и
приёмка реального Word на момент записи ещё не подтверждены.

C087 опубликован: PR132 merge 5e18292f270d9ca0d95d612477a58f8782b4fb05;
Safety35679522070, Pages35679909932 success. Три публичных файла совпали;
опубликованный модуль прошёл сценарии 30/300/1000 слов и пустого раздела.

## C-088 — проверка арифметики и контекст источников, 22.09.2026
R5/R7/R8/R9 → percentageCheck и sourceReview; отчёт редактора и итоговая проверка.
Явные процентные равенства проверяются без eval/AI с допуском округления;
нулевой знаменатель и ошибка блокируют путь приёмки. Связи текст/фрагмент/карточка
показаны явно, всегда требуют ручной смысловой проверки C10. Распознанные
ссылки не означают подтверждения смысла или подлинности. Предел — 200 пар с
явным счётчиком остатка. Область и ограничения: docs/C088_CONTENT_CHECK_SCOPE.md.
Локально npm test 331/331 + pretest13/13, quality42/42, diff --check чисто.
Добавлен browser-сценарий проверки арифметического запрета, отображения
несвязанного фрагмента и запрета C10 manual/fail; выполнение ожидается в CI.
Main сверена:5e18292f270d9ca0d95d612477a58f8782b4fb05; исходное дерево совпадает.
Рабочие данные, права, бюджет и платные вызовы не затронуты. CI/публикация и
полная содержательная приёмка на момент записи не подтверждены.

C088 первый CI35680997781: Node/SQL/A1/replay пройдены, browser94/95.
Новый сценарий выявил пропуск формулы после текстового двоеточия «Доля:».
Исправлен разбор границы: подпись допускается, числовой префикс деления
по-прежнему исключает частичное распознавание. Добавлены оба вида подписи
с пробелом/без пробела и отрицательные сложные префиксы. Quality42/42 прошёл;
проверки не отключались, browser/CI повторяются на новом коммите.

## C-089 — версионные замечания, 22.09.2026
R3/R5/R8/R9 → results-ui, requests/results, миграция c089_review_notes.
Отдельное сохранение fail/manual с местом и описанием; повторное открытие,
история последних 100 проверок всех версий. Положительный протокол остаётся строгим.
SQL блокирует выдачу по старой проверке после новых замечаний; старые версии
и замечания неизменяемы. Миграция использует существующие таблицы/права.
PGlite под service_role: сохранение/повтор, конфликт ID, запрет старого pass,
новый Word без приёмки старого, сохранность истории; anon/authenticated запрещены.
API: отрицательный результат восстанавливается, история ограничена заявкой и ролью.
Три целевых теста пройдены. Общий первый запуск 332/333: единственный отказ —
новая миграция ещё не внесена в manifest. Перечень обновлён с SHA256; повторяем.
Добавлен браузерный сценарий замечания → закрытие → восстановление → новая проверка.
AI-вызовов и передачи реального документа нет. CI/установка пока не подтверждены.

Повторный локальный npm test:334/334 + pretest13/13. Дерево загруженного PR134
совпало с локальным git write-tree. Перед установкой найден и исправлен остаточный
режим редактирования после новых замечаний к ранее положительной проверке;
добавлен browser-тест с потерей ответа, восстановлением и новым положительным ID.

## C-090 — частичная отправка, 22.09.2026
R3/R9 → index.html: предварительное чтение/проверка выбранных файлов, сохранение
серверного номера и перечня ожидаемых материалов до загрузки, явный статус после
ошибки и повторного открытия. Успех только после подтверждения всех ожидаемых
хешей/категорий; повтор пропускает уже подтверждённые файлы. Поля заявки сохраняются
после подтверждения update независимо от исхода загрузки. C080 freeze/CAS/права
сохранены; добавление материалов после начала подготовки по-прежнему запрещено.
Целевые Node/SQL тесты 9/9 пройдены. Добавлен browser-сценарий частичной загрузки,
закрытия формы, пустого повтора и загрузки остатка. Main сверена: 2e3026aa4e7b40ff4eb62501afb721e244fb1aff.
Общий CI и публикация пока не подтверждены. Нет новых серверных функций/миграций,
платных вызовов или изменения рабочих заявок. C11 Microsoft Word и выдача заявки
№12 остаются незавершёнными; этот патч не закрывает общую содержательную приёмку.
## C-091 — скачивание исходных вложений, 22.09.2026
R3/R9 → requests/index.ts и download-url.mjs. Рабочий интерфейс №9 показывает
четыре вложения, но скачивание возвращает общий отказ. Обнаружена несовместимость:
REST signedURL относителен к /storage/v1, прежний код принимал только полный
префикс. Подтверждено установленным storage-js и текущим официальным исходником
https://raw.githubusercontent.com/supabase/storage-js/master/src/packages/StorageFileApi.ts
(createSignedUrl). Проверен changelog Supabase; относящегося к этому формату
breaking change не найдено. Теперь оба относительных формата приводятся к URL
того же проекта и точного объекта; обязательны token и безопасное имя скачивания.
Права владельца/исполнителя, закрытый bucket, TTL и неизменяемость не меняются.
Локально npm test: 337/337 и pretest вложений 15/15. Целевой набор
attachments/requests/deploy: 45/45; git diff --check чисто.
База ветки — main b5b6bc24c278f98eebc5cde8eeb7423be9acd47c, fetch подтверждён.
GitHub create_branch и Supabase read-only запрос завершились транспортной ошибкой
HTTP 400 Invalid MCP request metadata. CI, установка и скачивание в рабочем
реестре не подтверждены; файлы/паспорта/исходные данные не менялись.

## C099 — тестовая передача без подтверждения качества
R3/R8/R9/M10. Отдельные grant и test-delivery, привязанные к заявке, получателю,
паспорту, версии и хешам. Обычные reviews/results/status не меняются. Обе роли
видят предупреждение о незавершённой проверке качества. Grant закрыт от изменения
service_role; test RPC закрыты от anon/authenticated. Отзыв, stale и смена владельца
запрещают чтение. Штатное удаление учитывает отдельные записи и аудит.
Независимый review: исправлены stale-v1→v2 и новый operation ID при смене версии.
Целевые SQL/API/deploygate и manifest проверки успешны; 7 браузерных сценариев
добавлены. Публикация и фактическая передача/скачивание в рабочем кабинете пока
не выполнены. Платных запросов нет. Пройденный technical test не закрывает
оригинальность, Microsoft Word и содержательную приёмку.

## C100 — адресное переназначение №4
R3/R9 → административная операция переноса с аудитом и CAS; исторические файлы
не перезаписываются, создаётся только связь облачной карточки с существующим UUID.
План согласован 23.09.2026. Реализация в работе; локальные/CI проверки,
публикация, перенос рабочей записи и приёмка в кабинете пока не подтверждены.
Локально: npm test 430/430, отдельный pretest вложений 15/15; C100 SQL 6/6,
точное восстановление карточки и CAS API 6/6. Публичный SQL не содержит идентификаторов
рабочих аккаунтов: допуск пустой, административный, адресный. Новые обращения старого
владельца проверяют текущего владельца заявки; ранее выданные ссылки на файлы имеют
существующий TTL до 300 секунд. Реальные конкурентные проверки в CI ещё ожидаются.
Промежуточный результат 23.09.2026: реализация зафиксирована локальным коммитом
 a2d690ea33144083a22b44c4187ed7deee95556b. Независимый review: docs/C100_REVIEW.md,
критичных блокеров финального SQL не найдено. Native fixture исправлен на настоящий
requestWorkCard и проверен в PGlite; 8 конкурентных schedules ожидают PostgreSQL CI.
Отправка ветки в GitHub отклонена автоматической проверкой разрешений: требуется
явное подтверждение публикации изменения в указанный репозиторий. Read-only GitHub
проверка подтвердила public repository innaodincova-finpro/studkab и доступ push/admin;
повторной отправки в обход отказа не было. PR и CI не запущены, production не изменён.
Локальные browser tests не стартовали: отсутствует Chromium executable; загрузчик
дважды получил повреждённый/пустой архив, попытка остановлена. Это ограничение среды,
а не подтверждённое падение пользовательского сценария. Браузерная приёмка pending.
PR145/e79cbc5 CI35869177347: Node430/430, SQL safety, C096 и C100 native concurrency
(8 schedules), A1 пройдены. Полный replay применил миграции, но контрольный список
ожидал27 таблиц вместо29. Исправлен точный список и добавлена проверка отсутствия
прав API-ролей на C100 transfer/allowlist/audit mutation; повторный CI требуется.

## C101 — сохранение полностью отвеченных уточнений при адресном переносе
R3/R8/R9 → новая миграция `20260923144056_c101_answered_reassignment.sql`.
Допускаются только завершённые ответы исходного владельца на вопросы действующего
исполнителя. UUID, тексты, авторы и время ответов не переписываются; неизменяемый
аудит хранит количество и канонические хеши строк. Проверка material_manifest
принимает прежний ответ только по точной связи владельцев и совпадающему снимку.
Открытые, чужие и повреждённые уточнения запрещают перенос. Старые паспорта остаются
stale; новый паспорт требует повторной проверки и привязки answer_ids. Allowlist,
CAS, роли и остальные запреты C100 сохранены. Старые аудиты получают пустой снимок.

Локально: полный npm test 438/438 и отдельный pretest вложений 15/15; C101 SQL
8/8, C100 SQL 6/6, проверка manifest 2/2. Проверены неизменность истории, повтор
операции, закрытые права transfer для anon/authenticated/service_role, отказ
старому владельцу, запрет перезаписи ответа новым владельцем, повторная приёмка
и независимость хеша от часового пояса. Независимый review: docs/C101_REVIEW.md.
Четыре последовательных исхода native fixture проверены в PGlite; это не проверка
реальной конкуренции. Native PostgreSQL schedules включены в Safety и ожидают CI.
Установка, перенос рабочей заявки, browser-приёмка и публикация не выполнялись.

### C101 — подтверждённая установка и синтетический маршрут, 23.09.2026
PR146 объединён в main `b53f5cd333208718bf1fa60a6eacbed266d7c495`.
Safety35877309156, job107236463400 на проверенном head — success: Node438,
pretest15, browser125, четыре native PostgreSQL сценария C101. Миграция
зарегистрирована в production как `20260923145936`; SQL совпал точно, SHA256
`f4aa89bfba8daeb7d3c9fe255c8342b3e0f4deb210a2896f51324f1f6bafdf60`.

Подтверждён адресный перенос одной существующей синтетической заявки: ревизия
1→2, ревизия облака получателя38→39. Единственный ответ и его снимок совпали;
исходное облако и семь прежних карточек получателя сохранены, число заявок7.
Новый паспорт версии6 утверждён после повторной проверки: историческая ссылка
answer_id сохранена, material_manifest валиден. Тестовая передача Word версии19
и скачивание дали одинаковый SHA256
`5eda2a57275b5026b25313cf88e5ba36529b59fba9bbcabf81fda0319263953c`.
Обычных выдач и генераций этой заявки0. Это подтверждает тестовый маршрут и
точность файла, не завершение содержательной проверки качества или обычную выдачу.
Доказательства установки и маршрута переданы ведущим агентом; эта запись и
сверка manifest не выполняют повторный перенос или изменение production.

## C102 — контракт качества итогового документа, 23.09.2026

R13 вместе с R3–R9 → docs/QUALITY_DOCUMENT_CONTRACT.md; TASK и
QUALITY_RELEASE_PLAN дополнены, прежние требования и история сохранены.
Отдельно зафиксированы содержание/методология, данные/расчёты, источники и смысл,
аргументация, точный Word, независимая оценка, внутренние заимствования,
внешний антиплагиат, исправления и выдача. Никаких гарантий полной достоверности
автоматикой или официального процента внутреннего сравнения не добавлено.

| Требование | Реализация / версия | Проверка и фактический результат | Публикация / приёмка |
|---|---|---|---|
| QD01–QD07 — предметное качество и независимая оценка | Существующие R5–R8 и критериальный протокол; новый полный контракт документирован | Определены A-QD01–A-QD04/A-QD13; полная автоматическая или содержательная реализация не подтверждена | Не закрыто |
| QD08 — внутреннее сопоставление с ограниченным корпусом | Требование и границы минимального C102 зафиксированы | A-QD05/A-QD06 определены; исполнение новой реализации ещё не подтверждено | Код/CI/публикация/приёмка pending |
| QD09 — внешний PDF и точное основание порога | Ручной пилот сохранён, обязательная привязка к Word и паспорту уточнена | A-QD07/A-QD08 определены; настоящий внешний пилот не выполнен этой правкой | Не закрыто; платные синтетические проверки запрещены |
| QD10 — цикл и серверный допуск | Требования к журналу и запрету новых positive review/delivery зафиксированы | A-QD08–A-QD12 определены; старые версии/исторические скачивания/C099 должны сохраняться | Реализация и сквозная приёмка ещё не завершены |

Фактически выполнена документальная фиксация требований до программного изменения.
Новая проверка не объявлена установленной; данные, расходы и действующие файлы
результатов этим пакетом не меняются. Последующие доказательства кода, тестов,
публикации и пользовательской приёмки записываются отдельно с точной версией.

### C102 — локальная реализация и независимый review

Код минимального вертикального среза реализован локально: серверный неизменяемый
журнал `studkab_quality_evidence`, внутреннее сравнение exact stored Word с
ограниченным корпусом, ручной внешний PDF с основанием порога из паспорта,
серверные gates новых положительных проверок и обычной выдачи, UI сохранения
и повторной проверки. Полный предметный контракт QD01–QD07 этим не закрывается.

Независимый review: [docs/C102_REVIEW.md](docs/C102_REVIEW.md). Исправлены скрытое
исключение источников без извлечения, тупик повторной проверки после изменения
evidence и завышенная формулировка покрытия при лимите. Финальный targeted
запуск API + SQL + requests: 42/42; scanner отдельно 11/11. Browser-кейсы
прочитаны, их исполнение ожидается в CI. Новый код не объявлен опубликованным.

Локальный пример ограниченного scanner для синтетического Word №2 с SHA256
`5eda2a57275b5026b25313cf88e5ba36529b59fba9bbcabf81fda0319263953c`:
1319 токенов Word / 128 токенов фрагмента P2, совпадений 0, усечения нет,
`status=manual`, `officialOriginality=false`. Это не production scan,
не внешний отчёт и не положительная оценка качества. Частные тексты не публикуются.

Статусы разделены: требования зафиксированы; код локально реализован и проверен
в указанном объёме; полный локальный npm test до последних правок: 462/462,
pretest 15/15. После старта полного запуска добавлен один API-кейс смешанного
пригодного/неизвлечённого корпуса: API 6/6 targeted. Также изменён ACL/hash
миграции, после чего manifest 2/2 targeted. Полный набор на окончательном head
локально не повторялся; ожидаемый CI-набор — 463 теста. CI, установка и
сквозная приёмка ещё pending.
Внешний антиплагиат на синтетических данных за плату не выполнялся и запрещён.

### C102 — исправление адресной browser-проверки после первого CI

Safety run `35884478533`, job `107261000548`, head
`5f08ec98f83e81ab1249754440f5f814345c15c6`: 133 browser-сценария прошли,
один остановился на `toBeDisabled()` для `option[value=pass]`.
Причина установлена по исходнику установленного Playwright: `elementState`
использует `retarget(..., 'follow-label')`, поэтому option внутри label
перенаправляется на `label.control` — весь select. Select должен оставаться
доступным для результатов manual/fail; это не свидетельство снятия disabled
с самого option. `controls()` приложения не выбирает и не меняет option.

`tests/browser/quality-evidence.spec.cjs` проверяет теперь непосредственно
нативное свойство option через `toHaveJSProperty('disabled', true)`.
Сценарий дополнительно проверяет отсутствие scan, доступность pass после полного
scan, сброс ранее выбранного pass в manual и запрет pass после пустого,
частично пригодного или усечённого корпуса. Результаты manual/fail доступны,
готовность false, запрос сохранения положительной проверки не отправлялся.
Продуктовый код и серверные ограничения не изменены и не ослаблены.

Локально `node --check` успешен; Playwright `--list` собирает все шесть
сценариев файла. Это не исполнение browser-тестов: исправленная проверка
ожидает повторного CI. Установка и пользовательская приёмка ещё не выполнены.

### C102 — установленный минимальный срез и рабочая приёмка №2

PR147 merge `4f7ce596896741bf02378dfd6de592de12830786`; Safety35887007197:
463 Node / 134 browser, native concurrency, SQL/A1/replay — success.
Миграция20260923161812, Edge v55, Pages35887950450 установлены и проверены.
[Подробный протокол](docs/C102_LIVE_UAT.md): внутренний scan №2 сохранён
как manual и восстановлен; обычная выдача заблокирована; тестовый Word скачан
без изменения SHA256. Прежние данные и два manual-замечания сохранены.
Новый журнал содержит одну запись; платных проверок и обычных выдач нет.
Полный контракт качества и общий выпуск этим не закрыты.
# C107 — точный Word: честный шаг просмотра, 24.09.2026

R3/R8/R13 → `results-ui.js`, `reestr.html`, `index.html`, `sw.js` и `tests/browser/results.spec.cjs`. Локально изменены надпись кнопки, инструкция, отдельное подтверждение открытия скачанного файла и сброс подтверждения при изменении сохранённого Word. `npm test`: 472/472, `node --check` и `git diff --check` пройдены. Первый CI: 133 браузерных сценария прошли, два остановились на устаревшем ожидании текста и закрытом разделе новой проверки. Исправлены только эти два тестовых шага; повторный CI обязателен. Локально Chromium отсутствует. Рабочая публикация/приёмка пока не проведены. Заявка №4 и её серверные проверки этой правкой не изменены; утверждать, что замечания сохранены, нельзя.
# C108 — адресное обновление карточки перед исправлением вида, 24.09.2026

R3/R8/R9/R13 → `reestr.html`, `sw.js`, `tests/browser/registry-workspace.spec.cjs`.
В рабочей базе №3 уже имеет вид «Практическая работа», revision 10 и одну запись
истории исправления от 24.09, тогда как локальная карточка браузера показывала
«Контрольная работа»; повторная операция ответила конфликтом. В форме C105 при
обнаружении отставания сначала читается адресная строка `inbox` по ID, проверяется
точная редакция, затем обновляется локальная карточка с сохранением заметки и
документа. Запись исправления и импорт других кабинетов не вызываются. Браузерные
сценарии проверяют обновление и отказ от ответа с неверной редакцией. Локальные
проверки и CI фиксируются отдельно; рабочая публикация и приёмка ещё не проведены.

## C110 — восстановление сохранённого Word после локального расхождения

R3/R8/R9/R13 → `supabase/functions/studkab-requests/results.mjs`, `results-ui.js`,
`tests/requests.test.mjs`, `tests/browser/results.spec.cjs`, `reestr.html`,
`index.html`, `sw.js`. Сервер выдаёт только сохранённое описание того же Word при
действующей привязке; клиент сверяет исходный материал с утверждённым паспортом
перед повторным чтением состояния. Другой хеш/тема/получатель блокируются.
Локальные, CI и рабочие проверки фиксируются отдельно: без рабочей приёмки C110
не считается исправленным в приложении.

## C114 — повторный прогон: входная проверка, 25.09.2026

Старая учебная заявка UUID `6a8cbdda-833d-42fd-a34c-462cc701fa11`
удалена штатно в реестре; связанные 4 вложения и 2 паспорта отсутствуют,
запись удаления сохранена. Работа старого теста удалена в кабинете; там
отображалось 0 работ. На 19:41 UTC создана только **локальная новая работа**
«Финансовое планирование на предприятии — ТУСУР, учебный вариант 1».
В базе `studkab_requests` по-прежнему 0 строк; номер новой серверной заявки
ещё не присвоен, Telegram о ней не оповещён, паспорта, генерации и Word нет.
Последовательность номера адресно подготовлена к 1 при нулевом реестре;
последующая проверка показала `last_value=1, is_called=false`.

Первичный официальный PDF ТУСУР, 36 страниц, SHA256
`6d6fef5b90b3a5b35f2cd5de39f27730aac8809ab7a605913aa249d7a82a6426`.
Раздел 1 и приложения А/Б/Д дают вариант 1, тему, нормативы и числовые
исходные данные. Подготовлены четыре категории файлов: задание, PDF методики,
указатель исходных чисел, список 10 открытых публикаций. Ссылки на публикации
требуют окончательной сверки библиографических реквизитов, часть страниц
показывает CAPTCHA. Полное качество пакета ещё не принято.

**P1, подтверждено кодом:** `index.html:1116–1130` вызывает `submit` и
получает строку/номер до `uploadRequestFiles`. Очередь `claim_studkab_requests`
выбирает всякую строку с `telegram_sent_at IS NULL`, а `inbox` читает её сразу.
Если загрузка любого из четырёх файлов прервётся, исполнитель и Telegram
могут получить неполный комплект. Действующий серверный `validatePayload`
проверяет только тему и контакт (`handler.mjs`), наличие файлов не проверяет.
Это именно механизм прежнего нежелательного обхода, а не ошибка пользователя.
До атомарного завершения комплекта и задержки публикации заявку не подавать.

**P2, видно в рабочем кабинете:** вновь созданная курсовая получает
дополнительную «Главу 3. Результаты», которой нет в структуре варианта 1;
готовность 0%, ФИО/вуз/город/руководитель пока пусты. Параметры документа
не сопоставлены с официальной методичкой. Это отдельные риски качества Word,
их нужно проверить в паспорте и конечном документе после устранения P1.

Автоматическая проверка разрешений отклонила клик по «Отправить заявку»
на пустой работе как потенциальную отправку неполной заявки. Чтение исходника
показало, что первый клик только открывает форму, но повторять действие через
обход проверки нельзя. Ни один `submit` не вызывался. Маршрут остановлен на
входном блокере; прохождение реестра, паспорта, качества и возврата Word не
заявляется.

### C114 — локальная правка входного блокера

R3/R5/R9 → новая миграция `20260925194500_c114_complete_before_publish.sql`,
`handler.mjs`, `index.html`. Серверный `ready_at` отделяет созданный черновик
от видимой заявки, `studkab_request_publish` проверяет по серверным метаданным
ровно четыре текущие категории и открывает запись только после сохранения
последнего файла. `claim_studkab_requests` и `inbox` читают только опубликованные
строки. Клиент предварительно выбирает четыре файла, загружает их и лишь потом
вызывает `request-publish`; ошибку загрузки обозначает как незавершённый
черновик. Старые страницы отправки отвергаются по `materialsFlow=2`.
Локально тест RPC/очереди через PGlite проверил отсутствие уведомления до
четвёртого вложения и единственную публикацию после него (1/1); весь
`npm test`: предварительные 15/15, основной набор 485/485; после адресной
защиты паспорта на неопубликованной заявке дополнительно 49/49 профильных
тестов. Браузерная проверка, публикация и рабочая приёмка ещё не выполнены.
До публикации это не исправление работающего сайта.

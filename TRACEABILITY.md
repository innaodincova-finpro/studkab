...
Браузерные 6 сценариев не запущены из-за отсутствия Chromium в рабочей среде,
поэтому UI-приёмка не подтверждена. Код в локальной ветке
`codex/c103-real-request-quality` (коммиты `47e72c4` и `0372ecc`);
автоматическая проверка разрешений отклонила отправку в GitHub как публикацию
кода без явного согласия на эту операцию; удалённая ветка отсутствует.
...
Статус: исследование и локальная правка; установки и полной предметной
приёмки нет. Нерешённая зависимость: C102 пока требует внешний PDF с числовым
порогом даже при отсутствии вузовского числа. Пакет SQL/Edge/UI и проверки
«нет правила / есть порог / конфликт / правило без порога» ещё нужен; старые
свидетельства и обычный допуск не переписывались.

...
Код подготовлен; фактический результат и ссылка на Safety run будут добавлены после PR.
Это не выдаётся за отключение размещённого Supabase Edge runtime. Протокол:
`supabase/A1_FAULT_INJECTION_PROTOCOL.md`.

...
R3/R5/R8/R9 → внешний .docx до 3 МБ сохраняется отдельной неизменяемой версией;
проверка ZIP/XML, хеша и извлечённого текста на клиенте и сервере. Редактор не заменяется.
Передача требует утверждённого паспорта и прежнего протокола 16 критериев.
Локально: npm test — 318/318, pretest вложений 13/13; отдельная проверка маршрута
внешнего Word — 3/3. Подготовленный MGMT02 Word прочитан без изменения байтов.
Браузерные сценарии добавлены; локально не запущены: Chromium не скачался.
Публикация и производственная сквозная приёмка не выполнены. Проверка DOCX
ограниченная структурная, не антивирус и не содержательная экспертиза документа.

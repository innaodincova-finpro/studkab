(function(global){
 ...
 function check(x){
  ...
  var w=openModal('<button type="button" class="close" data-x="1">✕</button><h3>Проверка черновика</h3>'+notesHtml+ (errors.length?'<p>Передача пока недоступна:</p><ul>'+errors.map(function(e){return '<li>'+esc(e)+'</li>';}).join('')+'</ul>':'<p>Автоматическая проверка не обнаружила пустых разделов и известных пометок. Она не подтверждает содержание документа.</p><p>Следующий шаг — итоговая проверка Word по отдельным критериям и с доказательством по каждому пункту.</p><label><input type="checkbox" data-review> Автоматические замечания просмотрены</label><button type="button" class="btn" data-approve>Перейти к итоговой проверке</button>')+'<p data-review-status role="status"></p>');

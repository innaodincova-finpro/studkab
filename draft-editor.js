(function(global){
 ...
 function form(x){
  ...
  var html='<details open><summary>Материалы для подготовки</summary><p class="hint">Заполните один раз. Используйте материалы студента и проверенные источники. Текстовые файлы TXT можно загрузить в поле материалов. Библиографию программа включит в документ без выдуманных дополнений.</p>';
 ...
 function check(x){
  var errors=DraftQuality.preflight(x).concat(DraftQuality.issues(x.doc));
  if(x.doc.basis && x.doc.basis!==basis(x))errors.push('Материалы или структура изменились после подготовки');
  var w=openModal('<button type="button" class="close" data-x="1">✕</button><h3>Проверка черновика</h3>'+ (errors.length?'<p>Передача пока недоступна:</p><ul>'+errors.map(function(e){return '<li>'+esc(e)+'</li>';}).join('')+'</ul>':'<p>Автоматическая проверка не обнаружила пустых разделов и известных пометок. Она не подтверждает достоверность исследования.</p><p>Проверьте Word: выводы соответствуют данным, ссылки ведут к использованным источникам, требования преподавателя выполнены.</p><label><input type="checkbox" data-review> Я проверил расчёты, факты, источники и требования</label><button type="button" class="btn" data-approve>Подтвердить проверку</button>')+'<p data-review-status role="status"></p>');

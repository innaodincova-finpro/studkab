(function(global){
 ...
 function form(x){
  var p=editableInputs(x),fields=[['organization','Организация или объект исследования'],['period','Период исследования'],['requirements','Задание и требования преподавателя'],['materials','Фактические материалы исследования'],['sources','Карточки проверенных источников']];
  var html='<details><summary>Материалы для подготовки</summary><p class="hint">Заполните один раз. Для каждого источника укажите обозначение [S1], строку «Реквизиты:», точную строку «Фрагмент:» и строку «Подтверждает:». Без проверяемого фрагмента и связи с утверждением передача будет заблокирована. Библиографию программа включит без выдуманных дополнений.</p>';
 ...
 function check(x){
  var acceptance=DraftQuality.documentAcceptance(x),errors=DraftQuality.preflight(x).concat(DraftQuality.issues(x.doc),DraftQuality.sourceCheck(x).errors,DraftQuality.consistency(x).errors,DraftQuality.proseIntegrity(x).errors,acceptance.errors,DraftQuality.finAcceptance(x).errors);
  ...
  var facts='<p class="hint">Найдено в документе: разделов — '+acceptance.facts.sections.length+', таблиц — '+acceptance.facts.tables+', рисунков — '+acceptance.facts.figures+', приложений — '+acceptance.facts.appendices+'.</p>';
  var w=openModal('<button type="button" class="close" data-x="1">✕</button><h3>Проверка черновика</h3>'+facts+notesHtml+ (errors.length?'<p>Передача пока недоступна:</p><ul>'+errors.map(function(e){return '<li>'+esc(e)+'</li>';}).join('')+'</ul>':'<p>Измеримые требования заявки и методички выполнены. Содержание документа автоматически не подтверждено.</p><p>Следующий шаг — итоговая проверка Word по отдельным критериям и с доказательством по каждому пункту.</p><label><input type="checkbox" data-review> Автоматические замечания просмотрены</label><button type="button" class="btn" data-approve>Перейти к итоговой проверке</button>')+'<p data-review-status role="status"></p>');

(function(global){
 ...
 function check(x){
  ...
  var groups=report.groups.filter(function(g){return g.errors.length;}).map(function(g){return '<details><summary>'+esc(g.title)+' · '+g.errors.length+'</summary>'+list(g.errors)+'</details>';}).join('');

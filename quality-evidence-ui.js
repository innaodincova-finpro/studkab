 ...
 function mount(host,options){
  ...
  function render(){
   ...
   body.querySelector('[data-quality-save-external]').onclick=function(){return run(async function(){
    ...
    ['thresholdPercent','actualPercent'].forEach(function(k){if(value(k).value==='')throw validation('Укажите результат и подтверждённый порог.');payload[k]=Number(value(k).value);if(!Number.isFinite(payload[k])||payload[k]<0||payload[k]>100)throw validation('Процент должен быть от 0 до 100.');});
    ...
    if(payload.disposition==='pass'&&payload.actualPercent<payload.thresholdPercent)throw validation('Результат ниже требуемого порога. Сохраните замечание, затем доработайте Word.');
